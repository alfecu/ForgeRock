...
define(["jquery", "underscore", "handlebars", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/openidm/ui/common/delegates/SearchDelegate", "org/forgerock/openidm/ui/common/delegates/ResourceDelegate", "org/forgerock/openidm/ui/admin/delegates/ConnectorDelegate", "org/forgerock/openidm/ui/common/delegates/ConfigDelegate", "org/forgerock/openidm/ui/common/delegates/SchemaDelegate"], function ($, _, Handlebars, constants, eventManager, searchDelegate, ResourceDelegate, ConnectorDelegate, ConfigDelegate, SchemaDelegate) {
    var obj = {};
    ...
    obj.getDisplayText = function (prop, item, resourceCollectionIndex) {
        ...
        return Handlebars.Utils.escapeExpression(txt);
    };
    ...
    obj.autocompleteProps = function (prop, resourceCollectionIndex, showRaw, item) {
        ...
        */
        if (!fields.length && item) {
            _.forEach(_.omit(item, "_id", "_rev", "_ref", "_refProperties"), function (val, key) {
    ...
    obj.setupAutocompleteField = function (autocompleteField, prop, opts, resourceCollectionIndex, propertyValue) {
        return obj.getAllAvailableResourceCollections().then(function (allAvailableResourceCollections) {
                ...
                load: function load(query, callback) {
                    ...
                    searchDelegate.searchResults(pathToResource, obj.autocompleteProps(prop, resourceCollectionIndex, true), query, null, queryFilter).then(function (result) {
                        var convertNestedProps = function convertNestedProps(item) {
                            _.forEach(obj.autocompleteProps(prop, resourceCollectionIndex), function (propName) {
            ...
            autocompleteField.selectize(_.assignIn({}, defaultOpts, opts || {}));
        });
    };
    ...
    obj.convertRelationshipTypes = function (properties) {
        _.forEach(properties, function (prop, key) {
    ...
    obj.getAllAvailableResourceCollections = function () {
        var currentConnectorsPromise = ConnectorDelegate.currentConnectors(),
            ...
            internalRoleSchemaPromise = SchemaDelegate.getSchema("internal/role"),
            resourceCollections = [];

        return $.when(currentConnectorsPromise, managedPromise, internalRoleSchemaPromise).then(_.bind(function (connectors, managedObjects, internalRoleSchemaResponse) {
            var connectorConfigPromises = [],
                internalRoleSchema = internalRoleSchemaResponse[0],
                internalRoleFields = _.filter(internalRoleSchema.order, function (propName) {
                var property = internalRoleSchema.properties[propName];
                return property.type === "string" && !property.encryption && property.scope !== "private" && !property.isVirtual;
            });

            if (internalRoleFields.length > 4) {
                internalRoleFields = internalRoleFields.slice(0, 4);
            }

            _.forEach(managedObjects.objects, _.bind(function (managed) {
            ...
            resourceCollections.push({
                path: "internal/role",
                label: internalRoleSchema.title,
                query: {
                    queryFilter: "true",
            ...
            if (!connectors.length) {
                ...
                _.forEach(connectors, _.bind(function (connector) {
                    ...
                    connectorConfigPromises.push(connectorConfigPromise.then(function (connectorConfig) {
                        _.forEach(connectorConfig.objectTypes, function (otSchema, ot) {
                            var fields = [];

                            if (ot !== "__ALL__") {
                                _.forEach(otSchema.properties, function (property, key) {
