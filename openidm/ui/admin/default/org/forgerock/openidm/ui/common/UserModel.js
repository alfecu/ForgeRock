...
define(["jquery", "underscore", "org/forgerock/commons/ui/common/main/AbstractModel", "org/forgerock/commons/ui/common/main/Configuration", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/commons/ui/common/main/EventManager"], function ($, _, AbstractModel, Configuration, Constants, EventManager) {
    ...
    UserModel = AbstractModel.extend({
        ...
        sync: function sync(method, model, options) {
            ...
            if (method === "update" || method === "patch") {
                if (this.currentPassword !== undefined) {
                    headers[Constants.HEADER_PARAM_REAUTH] = this.encodeRFC5987IfNecessary(this.currentPassword);
                    _.assignIn(options, { "headers": headers });
                }
        ...
        getUIRoles: function getUIRoles(roles) {
            ...
            _.forEach(Configuration.globalData.roles, function (value, key) {
                if (key.indexOf("/") === -1) {
        ...
        login: function login(username, password) {
            ...
            var headers = {};
            headers[Constants.HEADER_PARAM_USERNAME] = this.encodeRFC5987IfNecessary(username);
            headers[Constants.HEADER_PARAM_PASSWORD] = this.encodeRFC5987IfNecessary(password);
        ...
        setAuthTokenHeaders: function setAuthTokenHeaders(currentHeaders, authDetails) {
            ...
            if (authDetails) {
                ...
                updatedHeaders = _.assignIn(updatedHeaders, (_$assignIn = {}, _defineProperty(_$assignIn, "X-OpenIDM-OAuth-Login", true), _defineProperty(_$assignIn, "X-OpenIDM-DataStoreToken", authDetails), _$assignIn));
            } else {
        ...
        getProfile: function getProfile(headers) {
            ...
            return ServiceInvokerTarget.restCall({
                ...
                */
                if (_.includes(sessionDetails.authorization.roles, "internal/role/openidm-authenticated")) {
                    /*
        ...
        encodeRFC5987IfNecessary: function encodeRFC5987IfNecessary(headerValue) {
            var encoded = encodeURIComponent(headerValue).replace(/['()]/g, escape).replace(/\*/g, "%2A").replace(/%(?:7C|60|5E)/g, unescape);
